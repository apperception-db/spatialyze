...
class GeospatialVideo:
    def __init__(
        ...
        if isinstance(camera, str):
            ...
            self.camera = [_camera_config(c, idx) for idx, c in enumerate(camera_configs)]
        else:
            self.camera = camera
        self.keep = keep


def _camera_config(c: "Any", idx: int):
    assert isinstance(c, (list, tuple)), c
    assert len(c) == 13, c
    c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, c11, c12, c13 = c
    return camera_config(c1, c2, idx, c4, c5, c6, c7, c8, c9, c10, c11, c12, c13, 0)
